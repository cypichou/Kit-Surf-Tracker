 ...
 */
void GPS_DEMO(gps_datas_t *gps_datas)
{
	uint8_t c;
	...
	while(UART_data_ready(UART1_ID))
	{
		...
		if(GPS_process_rx(c, gps_datas) == TRAME_GPRMC)
		{
			...
			printf("UART3 : latitude=%lf,longitude= %lf\n",gps_datas->latitude_deg, gps_datas->longitude_deg);
		}

	}
}
/*
 * Cette fonction blocante permet de calculer la vitesse à laquelle l'antenne gps va.
 * @param *gps_datas	:pointeur vers une structure de doonée gps.
 * @param *temps : pointeur vers une valeur qui represente le temps depuis le dernier appel de la fonction.
 * @ret : la vitesse en entier
 */
int GPS_VITESSE(gps_datas_t *gps_datas) // il va falloir qu'il retourne qqch
{
	uint8_t c;
	...
	while(UART_data_ready(UART1_ID))
	{
		...
		if(GPS_process_rx(c, gps_datas) == TRAME_GPRMC)
		{
			...
			static bool_e firstEntrance = FALSE;

			L1 = gps_datas->latitude_deg;
			g1 = gps_datas->longitude_deg;
			static float L2 = 0.0;
			static float g2 = 0.0;
			double distance = 0.0;
			double vitesse_fct = 0.0;

			if(firstEntrance)
			{

				double sommeL2_L1 = 0.0;
				double sommeG2_G1 = 0.0;

				sommeL2_L1 =qfp_fsub(L2,L1);

				distance = (qfp_fsin(sommeL2_L1)/2.0) * (qfp_fsin(sommeL2_L1)/2.0);

				sommeG2_G1 = qfp_fsub(g2, g1);

				distance = distance + (qfp_fcos(L1) * qfp_fcos(L2) * qfp_fsin((sommeG2_G1)/2.0) * qfp_fsin((sommeG2_G1)/2.0));

				distance =  2 * qfp_fatan2(qfp_fsqrt(distance),qfp_fsqrt(1-distance));
			...
			vitesse_fct = distance / (1000/3600000.0); // 10/3 600 000 conversion en heure pour avoir du km/h

			vitesseI = qfp_double2int(vitesse_fct);
